...
def decompose_mp_sample(number_components, data, sample_size=1, threads_sample=1, iterations=100, decay_rate_mask = 0.5,
    ...
    #sample = [number_components for i in range(sample_size)]
    sample = [i for i in range(sample_size)] #i is seed: 0 to sample_size
...
def decompose_mp(seed, data, iterations=100, decay_rate_mask = 0.5, decay_rate_lr = 0.5, number_components=[],
            decay_iterations = 5, decay_type_mask = 'exponential', decay_type_lr = 'exponential',
            learning_rate=0.2, mask=0.2, mask_cross_validation = 0.1, cut_cv_mask=0, batch_size=20, sliceTCA=True, positive=False,
            orthogonal_penalty=False, orthogonal_constraint = False, metric=False, cross_validation=True,
            precision=torch.float32, initialization='uniform', orthogonal_skip=(), verbose_train=False,
    ...
    if number_components == [0 for i in range(len(number_components))]:
        ...
        if cross_validation:
            ...
            if cut_cv_mask==0:
                loss_cv = torch.sqrt(torch.mean(((1-mask_cross_validation[seed])*data)**2)/cv_mask_size).item()
            else:    
        ...
        _,_, ls, l_cv, _, model = decompose(seed=seed, data=data, number_components= number_components,
            ...
            decay_type_lr = decay_type_lr, learning_rate = learning_rate, mask = mask, cut_cv_mask=cut_cv_mask,
...
def grid_search_sample(data, max_subranks, min_subranks=(), sample_size=1, threads_sample=1, threads_grid=1, iterations=100,
            decay_rate_mask = 0.5, decay_rate_lr = 0.5, decay_iterations = 5, decay_type_mask = 'exponential',
            decay_type_lr = 'exponential', seed= random.randint(1,10**4), learning_rate=0.2, mask=0.2, cut_cv_mask=0,
            mask_cross_validation = 0.1, batch_size=20, sliceTCA=True, positive=False, orthogonal_penalty=False,
            orthogonal_constraint = False, metric=False, cross_validation=True, precision=torch.float32,
            initialization='uniform', orthogonal_skip=(), verbose_train=False, verbose_test=False, folder=None,
    ...
    if min_subranks==():
        min_subranks = [0 for i in max_subranks]
    max_subranks = [i+1 for i in max_subranks]
    rank_spam = [max_subranks[i]-min_subranks[i] for i in range(len(max_subranks))]
    valence = len(rank_spam)

    grid = get_grid_sample(min_subranks, max_subranks)
    ...
    ls_grid = ls_grid.reshape(rank_spam+[sample_size]).mean(dim=-1)
    seeds = seed_grid.reshape(rank_spam+[sample_size])

    if number_solutions==True:
        solutions = nb_solutions_grid.reshape(rank_spam+[sample_size]).permute([(valence+i)%(valence+1) for i in range(valence+1)])
        solutions = solutions[0].int()
